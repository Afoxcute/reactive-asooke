...
contract AsoEbiAution is Ownable(msg.sender), ReentrancyGuard, IERC721Receiver, IReactiveCallback, IReactiveCrossChain {
    // ===== ERROR ====
    ...
    // Cross-chain auction tracking
    mapping(uint256 => mapping(uint256 => bool)) public crossChainAuctions;

    // Events for cross-chain functionality
    event CrossChainAuctionCreated(uint256 indexed sourceChainId, uint256 indexed auctionId);
    event CrossChainBidPlaced(uint256 indexed sourceChainId, uint256 indexed auctionId, address bidder);
    ...
    // Handle callbacks for cross-chain bid verification
    function onCallback(bytes32 requestId, bytes memory response) external override {
        ...
        (bool valid, uint256 auctionId, address bidder, uint256 amount) = 
            abi.decode(response, (bool, uint256, address, uint256));
            
        if (valid) {
            _processCrossChainBid(auctionId, bidder, amount);
        }
    }
    ...
    function createCrossChainAuction(
        ...
        reactiveCrossChainProxy.sendMessage(
            targetChainId,
            address(this),
            "processCrossChainAuction(bytes)",
            params
        );

        emit CrossChainAuctionCreated(targetChainId, _tokenId);
    }

    // Place cross-chain bid
    function placeCrossChainBid(
        uint256 targetChainId,
        address _nftAddress,
        uint256 _tokenId
    ) external payable {
        bytes memory params = abi.encode(_nftAddress, _tokenId, msg.sender, msg.value);
        
        reactiveCrossChainProxy.sendMessage(
            targetChainId,
            address(this),
            "processCrossChainBid(bytes)",
            params
        );
    }
}
