 ...
 */
contract Escrow is ReentrancyGuard, IERC721Receiver, Ownable(msg.sender), IReactiveCallback {
    ...
    // Add Reactive Network callback handling
    function onCallback(bytes32 requestId, bytes memory response) external override {
        require(msg.sender == address(reactiveCallbackProxy), "Invalid callback");
        
        // Parse response and handle order completion
        (bool success, address buyer, address seller, uint256 orderId) = 
            abi.decode(response, (bool, address, address, uint256));
    ...
    // Add cross-chain order verification
    function verifyOrderOnSourceChain(uint256 orderId) internal {
        bytes memory params = abi.encode(orderId);
        reactiveCallbackProxy.requestCallback(
            sourceChainId,
