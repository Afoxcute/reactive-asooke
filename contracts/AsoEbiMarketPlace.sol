...
import {IReactiveCallback} from "@reactive-network/contracts/interfaces/IReactiveCallback.sol";
import {IReactiveCrossChain} from "@reactive-network/contracts/interfaces/IReactiveCrossChain.sol";
    ...
    IReactiveCrossChain
{
    ...
    // Add Reactive Network state variables
    uint256 public sourceChainId;
    address public reactiveCallbackProxy;
    address public reactiveCrossChainProxy;

    // Add cross-chain order tracking
    mapping(uint256 => mapping(uint256 => bool)) public crossChainOrders; // chainId => orderId => exists

    // Add events for cross-chain functionality
    event CrossChainOrderCreated(uint256 indexed sourceChainId, uint256 indexed orderId);
    event CrossChainOrderFulfilled(uint256 indexed sourceChainId, uint256 indexed orderId);
    ...
    // Handle callbacks from other chains
    function onCallback(bytes32 requestId, bytes memory response) external override {
        require(msg.sender == reactiveCallbackProxy, "Invalid callback");
        
        (bool success, uint256 orderId, address buyer) = 
            abi.decode(response, (bool, uint256, address));
            
        if (success) {
            fulfillCrossChainOrder(orderId, buyer);
        }
    }
    ...
    function createCrossChainOrder(
        ...
        uint256 _tokenId,
        uint256 _quantity,
        string memory _shippingInfo
    ) external payable {
        // Verify order parameters
        require(_quantity > 0, "Invalid quantity");
        
        // Create order on target chain
        bytes memory params = abi.encode(_tokenId, _quantity, msg.sender, _shippingInfo);
        
        reactiveCrossChainProxy.sendMessage(
            targetChainId,
            address(this),
            "processCrossChainOrder(bytes)",
            params
        );

        emit CrossChainOrderCreated(targetChainId, nextOrderId++);
    }

    // Process incoming cross-chain orders
    function processCrossChainOrder(bytes memory params) external {
        require(msg.sender == reactiveCrossChainProxy, "Invalid cross-chain call");
        
        (uint256 tokenId, uint256 quantity, address buyer, string memory shippingInfo) = 
            abi.decode(params, (uint256, uint256, address, string));

        // Create the order
        _createOrder(tokenId, quantity, buyer, shippingInfo);
    }

    // Verify order status on source chain
    function verifyOrder(uint256 orderId) external view returns (bool) {
